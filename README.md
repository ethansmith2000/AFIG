...
The corresponding ordered prior uses shifted teacher forcing:

```text
[BOS, z1, ..., z31] -> causal Transformer -> conditions for [z1, ..., z32]
```

Each 64-D next token is generated by a shared six-block conditional rectified-
flow head. The head fuses the causal state and timestep through a two-layer MLP
before AdaLN-Zero modulation. Training and inference use the identical BOS
alignment; no CFG or EMA is used.
