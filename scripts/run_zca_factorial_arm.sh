#!/usr/bin/env bash
# One queue-managed train-plus-FID5k arm of the axial-ZCA factorial.
set -euo pipefail
...
cache="${cache_root}/latents_axial_zca_g1_original_flip.pt"
cache_ready="${cache_root}/zca_cache_g1_ready"
cache_failed="${cache_root}/zca_cache_g1_failed"
protocol="reports/2026-08-26_autoencoder_program/zca_whitening_protocol.json"
while [[ ! -f "$cache_ready" ]]; do
  if [[ -f "$cache_failed" ]]; then
    echo "${arm} cancelled because ZCA cache construction failed" >&2
    exit 1
  fi
  sleep 30
done
...
case "$arm" in
  ...
  ordered_uniform)
    ...
    ;;
  common_weighted)
    name="v37-v27-axial-zca1-common-flowweight-prior-s1"
    objective_args=(
...
if [[ ! -f "${prior}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${name}-train" --wait -- \
    python -u train_progressive_joint_flow.py \
      ...
      --run_group autoencoder-v27-zca-factorial \
...
echo "ZCA FACTORIAL ${arm} COMPLETE $(date -u +%FT%TZ)"
