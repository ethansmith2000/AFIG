...
output_dir=${project_root}/continuous_runs/ar_fft_cartesian_ecs_snr_slim_10k

cd "${project_root}"
exec /workspace/bin/gpu-claim run \
  --owner AFIG \
  --job ar_fft_cartesian_ecs_snr-slim-10000 \
  --wait \
  -- \
  ...
  --run_name phase-a-ar-fft-cartesian-ecs-snr-slim-10k
