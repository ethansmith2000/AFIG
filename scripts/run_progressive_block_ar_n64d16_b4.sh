...
output=prior_runs/v2-block-ar-cross-n64-d16-b4-s1

gpu-claim run \
  --owner AFIG \
  --job block-ar-cross-n64-d16-b4-60k \
  --wait -- \
  /venv/main/bin/python -u train_progressive_ar_flow.py \
    ...
    --run_group ar-prior-block-factorization \
    --run_name v2-block-ar-cross-n64-d16-b4-s1 \
    --compile

gpu-claim run \
  --owner AFIG \
  --job block-ar-eval60k-cross-n64-d16-b4 \
  --wait -- \
  /venv/main/bin/python -u evaluate_progressive_joint_flow.py \
    --checkpoint "$output/checkpoint_final.pt" \
    --output_dir prior_evals/v2-block-ar-cross-n64-d16-b4-060000 \
    --num_samples 5000 \
    --batch_size 256 \
    --sample_steps 50
