#!/usr/bin/env bash
# Third-tokenizer-seed Stage-A screen: cross, residual, and true register tokens.
set -euo pipefail
...
common_train_args=(
  ...
  --run_group autoencoder-stage-a-tokenizer-seed3
  --compile
)

case "$arm" in
  v8)
    name="v8-unordered-vae-s3"
    architecture_args=(--encoder_depth 8 --pool_type cross_only --pool_depth 1)
    ;;
  v12)
    name="v12-unordered-vae-residual-e7p1-n64d16-s3"
    architecture_args=(--encoder_depth 7 --pool_type residual --pool_depth 1)
    ;;
  v13)
    name="v13-unordered-vae-register-e7j1-n64d16-s3"
    architecture_args=(--encoder_depth 7 --pool_type register_tokens --pool_depth 1)
    ;;
esac
...
# This machine is not volume-backed. Preserve every completed checkpoint on
# W&B, but do not let a telemetry outage erase the scientific evaluation.
...
echo "STAGE A TOKENIZER SEED 3 ${arm} COMPLETE $(date -u +%FT%TZ)"
