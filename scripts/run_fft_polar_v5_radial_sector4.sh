...
output_dir="${project_root}/continuous_runs/ar_fft_polar_v5_radial_sector4_logeps_s1_${steps}"
codec_stats="${project_root}/autoencoder_runs/codec_stats_32_global_standardize.pt"

cd "${project_root}"
exec /workspace/bin/gpu-claim run \
  --owner AFIG \
  --job "fft-polar-v5-radial-sector4-logeps-s1-${steps}" \
  --gpu "${gpu}" \
  --wait \
  -- \
  ...
  --generation_grouping radial_sector \
  --generation_group_size 4 \
  --diff_width 768 \
  --diff_depth 6 \
  --decoder_geometry factorized_polar \
  --factorized_coordinate_mode physical_standardized \
  --factorized_amplitude_transform log_eps \
  --factorized_amplitude_transform_parameter 1 \
  --factorized_log_epsilon 0.003 \
  --factorized_amplitude_standardization global \
  --factorized_amplitude_source_scale unit \
  --factorized_condition_fusion add \
  ...
  --run_name "fft-polar-v5-radial-sector4-logeps-s1-${steps}" \
  --run_group fft-polar-v5-token-composition-gate \
  --run_tags "direct-fft,polar-v5,radial-sector4,134-ar-steps,logeps-amplitude,unit-source,additive-condition,log-history,physical-standardized,isometric,velocity,intrinsic-phase,bernoulli-sign,cartesian-primary,fixed-dim,depth6,seed-1"
