#!/usr/bin/env bash
# Three-seed interaction test: true register-token formation x decoder jitter.
set -euo pipefail

if [[ $# -ne 1 || ( "$1" != "s1" && "$1" != "s2" && "$1" != "s3" ) ]]; then
...
seed="${1#s}"
tokenizer_name="v25-register-e7j1-det-jitter05-n64d16-s${seed}"
...
if [[ ! -f "${tokenizer}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${tokenizer_name}-tokenizer" --wait -- \
    python -u train_progressive_tokenizer.py \
      ...
      --run_group autoencoder-register-jitter-factorial \
...
# Reconstruction is only a permissive broken-codec veto. Generation selects.
...
then
  ...
  echo "REGISTER JITTER FACTORIAL s${seed} STOPPED AT CODEC HEALTH GATE $(date -u +%FT%TZ)"
  exit 0
fi
...
echo "REGISTER JITTER FACTORIAL s${seed} COMPLETE $(date -u +%FT%TZ)"
