#!/usr/bin/env bash
# End-to-end second-tokenizer-seed confirmation for the v13 register-token arm.
set -euo pipefail
...
tokenizer_name="v13-unordered-vae-register-e7j1-n64d16-s2"
tokenizer="tokenizer_runs/${tokenizer_name}"
cache="${tokenizer}/latents_final_original_flip.pt"
prior_name="v13-joint-register-e7j1-n64d16-tokenizer-s2-prior-s1"
...
if [[ ! -f "${tokenizer}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${tokenizer_name}-tokenizer" --wait -- \
    python -u train_progressive_tokenizer.py \
      ...
      --encoder_depth 7 --pool_type register_tokens --pool_depth 1 \
      --report_to wandb --tracker_project_name afig-progressive-tokenizer \
      --run_group autoencoder-stage-a-register-token-confirmation \
...
# Reconstruction is a permissive codec-health veto, not a selector. Stop only
# for non-finite/corrupted metrics or for jointly catastrophic PSNR and rFID.
python - "$tokenizer" <<'PY'
import json
import math
import sys
...
echo "V13 REGISTER TOKENIZER SEED 2 CONFIRMATION COMPLETE $(date -u +%FT%TZ)"
