...
arm="${1:?usage: run_fft_polar_v3_physical_gate.sh v-or-x0 [GPU] [STEPS]}"
gpu="${2:-any}"
steps="${3:-10000}"

case "${arm}" in
  v) amplitude_prediction_type="v_prediction" ;;
  x0) amplitude_prediction_type="x0" ;;
  *)
    echo "unknown amplitude-prediction arm: ${arm}" >&2
    exit 2
    ;;
esac

project_root=/workspace/AFIG
output_dir="${project_root}/continuous_runs/ar_fft_polar_v3_physical_amp${arm}_s1_${steps}"
codec_stats="${project_root}/autoencoder_runs/codec_stats_32_global_standardize.pt"

cd "${project_root}"
exec /workspace/bin/gpu-claim run \
  --owner AFIG \
  --job "fft-polar-v3-physical-amp${arm}-s1-${steps}" \
  --gpu "${gpu}" \
  --wait \
  -- \
  ...
  --factorized_amplitude_prediction_type "${amplitude_prediction_type}" \
  ...
  --run_name "fft-polar-v3-physical-amp${arm}-s1-${steps}" \
  --run_group fft-polar-v3-physical-gate \
  --run_tags "direct-fft,polar-v3,physical-standardized,isometric,amplitude-${arm},intrinsic-phase,bernoulli-sign,cartesian-primary,depth6,seed-1"
