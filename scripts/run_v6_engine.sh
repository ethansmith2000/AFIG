...
out="prior_runs/v6-${name}-vae-s1"
gpu-claim run --owner AFIG --job "v6-${name}-vae" --wait -- \
  python -u "$trainer" \
    ...
    --run_group diffusion-forcing-v6 --run_name "v6-${name}-vae-s1" \
    --compile "$@" > "${out}.launch.log" 2>&1 \
|| { echo "ENGINE ${name} FAILED"; exit 1; }
python -u scripts/backup_wandb_file.py "${out}/checkpoint_final.pt" "v6-${name}-vae" >> "${out}.launch.log" 2>&1
gpu-claim run --owner AFIG --job "v6-eval-${name}" --wait -- \
  python -u evaluate_progressive_joint_flow.py \
    --checkpoint "${out}/checkpoint_final.pt" \
    --output_dir "prior_evals/v6-${name}-vae-060000" \
    > "prior_evals/v6-${name}-vae-060000.log" 2>&1
