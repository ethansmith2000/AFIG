#!/usr/bin/env bash
# Posterior/noise parameterization arms on the selected v12 residual encoder.
set -euo pipefail

if [[ $# -ne 1 || ( "$1" != "det" && "$1" != "jitter05" && "$1" != "jitter10" && "$1" != "softvae" ) ]]; then
  echo "usage: $0 {det|jitter05|jitter10|softvae}" >&2
  exit 2
fi
...
case "$arm" in
  det)
    tokenizer_name="v19-residual-e7p1-det-n64d16-s2"
    posterior_args=(--no-variational)
    ;;
  jitter05)
    tokenizer_name="v20-residual-e7p1-det-jitter05-n64d16-s2"
    posterior_args=(--no-variational --decoder_jitter_std 0.05)
    ;;
  jitter10)
    tokenizer_name="v21-residual-e7p1-det-jitter10-n64d16-s2"
    posterior_args=(--no-variational --decoder_jitter_std 0.10)
    ;;
  softvae)
    tokenizer_name="v22-residual-e7p1-softvae-kl1e4-n64d16-s2"
    posterior_args=(--variational --kl_weight 1e-4 --log_variance_floor -8)
    ;;
esac
...
if [[ ! -f "${tokenizer}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${tokenizer_name}-tokenizer" --wait -- \
    python -u train_progressive_tokenizer.py \
      ...
      --run_group autoencoder-phase-c-posterior \
...
# Reconstruction is only a permissive codec-health veto. For the soft-VAE arm,
# a posterior concentrated within 0.05 logvar of the floor fails the named
# mechanism and is not advanced as another pseudo-deterministic control.
if ! python - "$tokenizer" "$arm" <<'PY'
import json
import math
import sys
...
record = {"codec_health": "pass", "psnr": psnr, "clean_rfid": rfid}
if arm == "softvae":
    posterior = metrics.get("posterior")
    if posterior is None:
        raise SystemExit("soft-VAE run did not report posterior statistics")
    floor_fraction = float(posterior["fraction_within_0p05_of_floor"])
    record.update(
        posterior_sigma_mean=float(posterior["sigma_mean"]),
        posterior_floor_fraction=floor_fraction,
    )
    if not math.isfinite(floor_fraction) or floor_fraction >= 0.95:
        raise SystemExit(
            f"soft posterior is boundary-pinned: floor_fraction={floor_fraction}"
        )
print(json.dumps(record, sort_keys=True))
PY
then
  touch "${tokenizer}/mechanism_gate_failed"
  echo "PHASE C POSTERIOR ${arm} STOPPED AT DECLARED HEALTH/MECHANISM GATE $(date -u +%FT%TZ)"
  exit 0
fi
...
if [[ ! -f "${evaluation}/metrics.json" ]]; then
  gpu-claim run --owner AFIG --job "${prior_name}-eval" --wait -- \
...
echo "PHASE C POSTERIOR ${arm} COMPLETE $(date -u +%FT%TZ)"
