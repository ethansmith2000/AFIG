#!/usr/bin/env bash
# Weak representation regularizers on the seed-1 residual+jitter backbone.
set -euo pipefail
...
case "$arm" in
  marginal)
    tokenizer_name="v26-residual-e7p1-det-jitter05-kurt1e4-n64d16-s1"
    regularizer_args=(--energy_reg_weight 1e-4)
    ;;
  slot)
    tokenizer_name="v27-residual-e7p1-det-jitter05-slotbal2e3-n64d16-s1"
    regularizer_args=(--slot_balance_weight 0.002)
    ;;
esac
...
if [[ ! -f "${tokenizer}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${tokenizer_name}-tokenizer" --wait -- \
    python -u train_progressive_tokenizer.py \
      ...
      --run_group autoencoder-representation-regularization \
...
then
  ...
  echo "REPRESENTATION REGULARIZER ${arm} STOPPED AT CODEC HEALTH GATE $(date -u +%FT%TZ)"
  exit 0
fi
...
echo "REPRESENTATION REGULARIZER ${arm} COMPLETE $(date -u +%FT%TZ)"
