#!/usr/bin/env bash
# Seed replication for the slot-balance representation regularizer.
set -euo pipefail

if [[ $# -ne 1 || ( "$1" != "s2" && "$1" != "s3" ) ]]; then
  echo "usage: $0 {s2|s3}" >&2
  exit 2
fi
...
seed="${1#s}"
tokenizer_name="v27-residual-e7p1-det-jitter05-slotbal2e3-n64d16-s${seed}"
...
if [[ ! -f "${tokenizer}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${tokenizer_name}-tokenizer" --wait -- \
    python -u train_progressive_tokenizer.py \
      ...
      --run_group autoencoder-representation-regularization-replication \
...
# Reconstruction remains a permissive health veto, never a representation rank.
...
then
  ...
  echo "SLOT BALANCE REPLICATION $1 STOPPED AT CODEC HEALTH GATE $(date -u +%FT%TZ)"
  exit 0
fi
...
echo "SLOT BALANCE REPLICATION $1 COMPLETE $(date -u +%FT%TZ)"
