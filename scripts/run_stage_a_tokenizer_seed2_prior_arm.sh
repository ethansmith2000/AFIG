#!/usr/bin/env bash
# Paired prior-seed-1 comparison on the completed tokenizer-seed-2 caches.
set -euo pipefail
...
case "$arm" in
  v8)
    tokenizer="tokenizer_runs/v8-unordered-vae-s2"
    name="v8-joint-unordered-vae-tokenizer-s2-prior-s1"
    ;;
  v12)
    tokenizer="tokenizer_runs/v12-unordered-vae-residual-e7p1-n64d16-s2"
...
if [[ ! -f "$cache" ]]; then
  echo "missing frozen tokenizer-seed-2 cache: $cache" >&2
  exit 1
fi
...
if [[ ! -f "${prior}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${name}-train" --wait -- \
    python -u train_progressive_joint_flow.py \
      ...
      --run_group autoencoder-stage-a-tokenizer-seed2-generative \
      --run_name "$name" \
...
echo "TOKENIZER SEED 2 MATCHED PRIOR ${arm} COMPLETE $(date -u +%FT%TZ)"
