#!/usr/bin/env bash
# End-to-end seed-1 comparison arm for the v13 register-token architecture.
set -euo pipefail
...
tokenizer_name="v13-unordered-vae-register-e7j1-n64d16-s1"
tokenizer="tokenizer_runs/${tokenizer_name}"
cache="${tokenizer}/latents_final_original_flip.pt"
prior_name="v13-joint-register-e7j1-n64d16-tokenizer-s1-prior-s1"
...
# Reconstruction remains a codec-health veto only; generation selects the arm.
python - "$tokenizer" <<'PY'
import json
import math
import sys
...
echo "V13 REGISTER TOKENIZER SEED 1 CONFIRMATION COMPLETE $(date -u +%FT%TZ)"
