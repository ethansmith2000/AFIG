#!/usr/bin/env bash
# Encoder-only fine-resolution arms on the selected v12 residual baseline.
set -euo pipefail

if [[ $# -ne 1 || ( "$1" != "direct2" && "$1" != "local2" ) ]]; then
  echo "usage: $0 {direct2|local2}" >&2
  exit 2
fi
...
case "$arm" in
  direct2)
    tokenizer_name="v14-residual-e7p1-encoder-p2-decoder-p4-n64d16-s2"
    stem_args=(--encoder_patch_size 2 --encoder_stem patch)
    ;;
  local2)
    tokenizer_name="v15-residual-e7p1-local-p2-to-p4-n64d16-s2"
    stem_args=(--encoder_patch_size 2 --encoder_stem fine_conv)
    ;;
esac

tokenizer="tokenizer_runs/${tokenizer_name}"
cache="${tokenizer}/latents_final_original_flip.pt"
prior_name="${tokenizer_name/tokenizer-/}-prior-s1"
...
if [[ ! -f "${tokenizer}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${tokenizer_name}-tokenizer" --wait -- \
    python -u train_progressive_tokenizer.py \
      ...
      --run_group autoencoder-stage-a-fine-stem \
...
# Reconstruction only vetoes a broken codec; the matched prior selects.
python - "$tokenizer" <<'PY'
import json
import math
import sys
...
print(json.dumps({"codec_health": "pass", "psnr": psnr, "clean_rfid": rfid}))
PY
...
echo "FINE STEM ${arm} COMPLETE $(date -u +%FT%TZ)"
