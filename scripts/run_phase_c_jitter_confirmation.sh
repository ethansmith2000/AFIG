#!/usr/bin/env bash
# Robustness confirmation for the Phase-C sigma-0.05 decoder-jitter candidate.
set -euo pipefail
...
case "$arm" in
  tokenizer_s1)
    tokenizer_name="v23-residual-e7p1-det-jitter05-n64d16-s1"
    tokenizer_seed=1
    ...
    ;;
  tokenizer_s3)
    tokenizer_name="v24-residual-e7p1-det-jitter05-n64d16-s3"
    tokenizer_seed=3
    prior_name="${tokenizer_name}-prior-s1"
    prior_seed=1
    train_tokenizer=true
    ;;
  prior2_v12)
    tokenizer_name="v12-unordered-vae-residual-e7p1-n64d16-s2"
    prior_name="v12-joint-residual-e7p1-n64d16-tokenizer-s2-prior-s2"
    prior_seed=2
    train_tokenizer=false
    ;;
  prior2_v20)
    tokenizer_name="v20-residual-e7p1-det-jitter05-n64d16-s2"
    prior_name="v20-residual-e7p1-det-jitter05-n64d16-s2-prior-s2"
    prior_seed=2
    train_tokenizer=false
    ;;
  *)
...
if [[ "$train_tokenizer" == true && ! -f "${tokenizer}/checkpoint_final.pt" ]]; then
  resume_args=()
  ...
  gpu-claim run --owner AFIG --job "${tokenizer_name}-tokenizer" --wait -- \
    python -u train_progressive_tokenizer.py \
      --seed "$tokenizer_seed" --objective full \
      ...
      --run_group autoencoder-phase-c-jitter-confirmation \
      --run_name "$tokenizer_name" --output_dir "$tokenizer" \
      --compile "${resume_args[@]}" >> "${tokenizer}.launch.log" 2>&1
fi

if [[ "$train_tokenizer" == true && ! -f "${tokenizer}/wandb_backup_attempted" ]]; then
...
if [[ ! -f "${tokenizer}/checkpoint_final.pt" ]]; then
  echo "missing tokenizer checkpoint: ${tokenizer}/checkpoint_final.pt" >&2
  exit 1
fi
...
if [[ "$train_tokenizer" == true && ! -f "${tokenizer}/axis_scorecard.json" ]]; then
...
if [[ "$train_tokenizer" == true && ! -f "${tokenizer}/decoder_sensitivity.json" ]]; then
...
# Reconstruction is a permissive broken-codec veto, never the promotion metric.
...
PY
  then
    ...
    echo "PHASE C JITTER CONFIRMATION ${arm} STOPPED AT CODEC HEALTH GATE $(date -u +%FT%TZ)"
    exit 0
  fi
fi
...
if [[ ! -f "${prior}/checkpoint_final.pt" ]]; then
  ...
  gpu-claim run --owner AFIG --job "${prior_name}-train" --wait -- \
    python -u train_progressive_joint_flow.py \
      --latent_cache "$cache" --output_dir "$prior" --seed "$prior_seed" \
...
echo "PHASE C JITTER CONFIRMATION ${arm} COMPLETE $(date -u +%FT%TZ)"
