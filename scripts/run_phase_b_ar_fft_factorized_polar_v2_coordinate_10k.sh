...
output_dir=${OUTPUT_DIR:-${project_root}/continuous_runs/ar_fft_factorized_polar_v2_eps01_global_10k}

cd "${project_root}"
exec /workspace/bin/gpu-claim run \
  --owner AFIG \
  --job "ar-fft-factorized-v2-coordinate-${phase_process}-10000" \
  --wait \
  -- \
  ...
  --run_name "phase-b-ar-fft-factorized-v2-eps01-global-${phase_process}-10k"
