...
tokenizer_run="tokenizer_runs/v2-cross-n64-d16-s1"
cache="${tokenizer_run}/latents_final_original_flip.pt"
output="prior_runs/v2-joint-cross-n64-d16-s1"
...
if [[ ! -f "$cache" ]]; then
  gpu-claim run \
    --owner AFIG \
    --job cache-cross-n64-d16-original-flip \
    --wait -- \
...
gpu-claim run \
  ...
  /venv/main/bin/python -u train_progressive_joint_flow.py \
    ...
    --run_name v2-joint-cross-n64-d16-s1 \
    --compile
