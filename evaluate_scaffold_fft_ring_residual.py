#!/usr/bin/env python3
"""Evaluate oracle scaffolds completed by a causal compact-FFT ring model."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--history_mode",
        choices=("sampled", "teacher_forced"),
        default="sampled",
        help="Diagnostic only: teacher_forced publishes real prior rings to history.",
    )
...
def main() -> None:
    ...
    if payload.get("kind") != "scaffold_fft_ring_residual":
        raise ValueError("checkpoint is not a scaffold FFT ring residual model")
    config = ScaffoldFFTRingConfig(**payload["model_config"])
    model = ScaffoldFFTRingModel(payload["scalar_ring"], config).to(device)
    ...
    for images, _ in loader:
        ...
        with torch.no_grad(), torch.autocast(
            ...
            if args.history_mode == "teacher_forced":
                true_residual = (
                    images.float() - scaffold - residual_mean
                ) / residual_std
                teacher_history_fft = images_to_fft_state(
                    codec,
                    true_residual,
                    layout_orbit,
                    layout_component,
                    int(saved["compact_token_dim"]),
                )
            else:
                teacher_history_fft = None
            sampled_flat = model.generate_fft(
                condition_patches,
                teacher_history_fft=teacher_history_fft,
                num_inference_steps=args.num_inference_steps,
                generator=generator,
            )
        normalized_residual = fft_state_to_images(
            codec, sampled_flat, layout_orbit, layout_component
        )
    ...
    metrics = {
        ...
        "completion": completion_accumulator.compute(
            reference,
            f"causal_fft_ring_completion_{args.condition_mode}_{args.history_mode}",
        ),
    }
